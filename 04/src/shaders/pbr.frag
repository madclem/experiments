...
void main() {
	...
	vec3 c = 1. * uBaseColor;
	// point light
	vec3 deltaPos = vec3( (uLight.xy - gl_FragCoord.xy) / uResolution.xy, uLight.z );
  vec3 lightDir = normalize(deltaPos);
  float lambert = diffuse(N, lightDir);

	float d = sqrt(dot(deltaPos, deltaPos));
	float att = 1.0 / ( .2 + (.2*d) + (.2*d*d) );

	c += (uLightColor.rgb * lambert * 2.0 * att);

	vec3 color 			= getPbr(N, V, c, uRoughness, uMetallic, uSpecular);
